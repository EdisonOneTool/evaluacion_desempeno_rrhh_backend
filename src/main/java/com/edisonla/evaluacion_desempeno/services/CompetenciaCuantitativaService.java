...
public class CompetenciaCuantitativaService {
    ...
    @Transactional
    public CompetenciaCuantitativaDto create(Long evaluacionId, CompetenciaCuantitativaRequest dto) {
    ...
    @Transactional
    public CompetenciaCuantitativaRequest update(Long evaluacionId, Long id, CompetenciaCuantitativaRequest dto) {
        CompetenciaCuantitativa cc = repository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Competencia Cuantitativa no encontrada: " + id));
        if(!cc.getEvaluacion().getId().equals(evaluacionId)) {
